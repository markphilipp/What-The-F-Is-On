...
namespace DataCompiler.Services
{
    public class DataLoader : IDataLoader
    {
        ...
        public void Run()
        {
            ...
            var cleanedModels = new List<MovieRating>();
            foreach (var model in models)
                cleanedModels.Add(CleanModelSources(model));

            _context.AddRange(cleanedModels);
            _context.SaveChanges();
        }

        private static MovieRating CleanModelSources(MovieRating model)
        {
            var sourceList = model.Sources.ToList();
            sourceList.RemoveAll(s => s == null);
            model.Sources = sourceList;
